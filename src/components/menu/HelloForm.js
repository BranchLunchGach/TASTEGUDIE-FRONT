...
const HelloForm = () => {
      ...
      const res = await axios.get(
        "/map-geocode/v2/geocode?query=" + encodeURIComponent(address),
        {
          headers: {
            ...
            "X-NCP-APIGW-API-KEY": process.env.REACT_APP_CLIENT_SECRET,
          },
        }
      );
  ...
  const handleStart = async () => {
    ...
    const validResults = geocodeResults.filter(
      (result) => result && result.longitude && result.latitude
    );
    ...
    // x, y 좌표 평균을 계산합니다.
    const avgX =
      validResults.reduce((sum, item) => sum + item.longitude, 0) /
      validResults.length;
    const avgY =
      validResults.reduce((sum, item) => sum + item.latitude, 0) /
      validResults.length;
  ...
  return (
    <div>
      <FirstContainer>
        ...
        >
          <SearchContainer>
            ...
            <LocationSearchDiv>
              <LocationAddBtn variant="primary" onClick={handleShow}>
                인원 추가하기
              </LocationAddBtn>
            </LocationSearchDiv>
          </SearchContainer>
      ...
      <Modal show={show} onHide={handleClose} centered>
        ...
        <Modal.Body>
          <LocationSearchDiv style={{ marginLeft: "40px" }}>
            ...
            />
            <LocationSearchBtn onClick={handleSearchClick}>
              검색하기
            </LocationSearchBtn>
          </LocationSearchDiv>
      ...
      <SecondContainer>
        <UserLocation>
          {locations.length === 0 ? (
            <p style={{ fontSize: "30px", color: "gray", textAlign: "center" }}>
              인원을 추가해주세요!
            </p>
          ) : (
            locations.map((location, index) => (
              <UserDiv key={index}>
                <ImgDiv imgUrl={images[index]} />{" "}
                {/* 이미지 배열의 인덱스에 따라 동적 배치 */}
        ...
        <GoBtn onClick={handleStart}>
          <span
            style={{ fontSize: "35px", fontFamily: "LOTTERIA CHAB-Regular" }}
          >
            출발
          </span>
          <br />
